...
- `middlewares/` exists with `log(logger)` (standard-logging observability factory), `tools(functions)` (register + inject OpenAI function schemas, with `register`/`unregister` for dynamic add/remove), and `compress(summarizer, *, threshold_chars, keep_recent)` (trace-start history compression via `before_trace` rewrite; the summary is merged into the system prompt so it stays first with no AI→System sequence; default summarizer uses `self.llm_client`); `multiagent/`, `cli/` not implemented yet. `examples/` and `docs/` exist.

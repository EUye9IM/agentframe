...
- **Persistence is not built in** — it's an escape hatch. `compile_kwargs` (e.g. `compile_kwargs={"checkpointer": InMemorySaver()}`) is passed verbatim to `workflow.compile(**...)`; `config` (`RunnableConfig | None`) on `invoke` is passed verbatim to the graph call. `session_id` is only a hook-carried key (`before_trace`/`after_trace`) — it is **not** mapped to a LangGraph `thread_id`, so history requires passing `config={"configurable": {"thread_id": ...}}` yourself (checkpointer restores state per thread — this **is** the multi-turn/multi-agent history mechanism).
...
- **Trace lifecycle**: `before_trace`/`after_trace` bracket `invoke` (the **only** public entry — text in, `str` out). `after_trace` returns the **last AI message's** content (empty string if none — a first-turn failure must not echo the user's own input back).
...
- Run all tests: `.venv/bin/python -m pytest tests/ -q` (currently 47, all pass).

...
- **Persistence is not built in** — it's an escape hatch. `compile_kwargs` (e.g. `compile_kwargs={"checkpointer": InMemorySaver()}`) is passed verbatim to `workflow.compile(**...)`; `config` on `invoke`/`stream`/`invoke_messages` is passed verbatim to the graph call. `session_id` is only a hook-carried key (`before_trace`/`after_trace`) — it is **not** mapped to a LangGraph `thread_id`, so checkpointer use requires passing `config={"configurable": {"thread_id": ...}}` yourself.

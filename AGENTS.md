...
- Run all tests: `.venv/bin/python -m pytest tests/ -q` (currently 57, all pass).
...
- `middlewares/` exists with `log(logger)` (standard-logging observability factory) and `tools(functions)` (register + inject OpenAI function schemas, with `register`/`unregister` for dynamic add/remove); `multiagent/`, `compression/`, `cli/` not implemented yet. `examples/` and `docs/` exist.
